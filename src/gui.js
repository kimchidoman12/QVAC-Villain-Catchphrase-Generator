#!/usr/bin/env node
// QVAC Villain Catchphrase Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29414;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/villainphrase") {
      try {
        const { motive } = await readBody(req);
        if (!motive || !motive.trim()) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Please fill in the field first" }));
          return;
        }
        const result = await generate(modelId, motive.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Villain Catchphrase Generator GUI ready at http://localhost:${PORT}`);
  });
